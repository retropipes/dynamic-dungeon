...
public abstract class SoundFactory extends Thread {
    ...
    public static SoundFactory getNonLoopingFile(final String file) {
        return SoundFactory.provisionMedia(new SoundFile(SoundFactory.MEDIA_GROUP,
                file, SoundFactory.ACTIVE_MEDIA_COUNT));
    }

    public static SoundFactory getNonLoopingResource(final URL resource) {
        return SoundFactory
                .provisionMedia(new SoundResource(SoundFactory.MEDIA_GROUP,
                        resource, SoundFactory.ACTIVE_MEDIA_COUNT));
    }
    ...
    static synchronized void taskCompleted(final int taskNum) {
        ...
        for (int z = taskNum + 1; z < SoundFactory.ACTIVE_MEDIA.length; z++) {
